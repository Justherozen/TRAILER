...
def get_dataset_trans(args):
    if args.dataset == 'cifar10':
        return get_cifar10_trans(args)
    elif args.dataset == 'cifar100':
        return get_cifar100_trans(args)
    elif args.dataset == 'svhn':
        return get_svhn(args)
    elif args.dataset == 'tinyimagenet':
        return get_tinyimagenet(args)
...
def get_cifar10_trans(args):
    ...
    test_dataset_known_trans = CIFAR10SSL_trans(args.data_root,train_unlabeled_idxs, train=True, transform=transform_val, download=False, labeled_set=list(range(0,args.no_known)))
    test_dataset_novel_trans = CIFAR10SSL_trans(args.data_root,train_unlabeled_idxs, train=True, transform=transform_val, download=False, labeled_set=list(range(args.no_known, args.no_class)))
    test_dataset_all_trans = CIFAR10SSL_trans(args.data_root,train_unlabeled_idxs, train=True, transform=transform_val, download=False)


    return train_labeled_dataset, train_unlabeled_dataset, train_pl_dataset, test_dataset_known, test_dataset_novel, test_dataset_all, test_dataset_known_trans, test_dataset_novel_trans, test_dataset_all_trans
...
def get_cifar100_trans(args):
    ...
    test_dataset_known_trans = CIFAR100SSL_trans(args.data_root, train_unlabeled_idxs,train=True, transform=transform_val, download=False, labeled_set=list(range(0,args.no_known)))
    test_dataset_novel_trans = CIFAR100SSL_trans(args.data_root, train_unlabeled_idxs,train=True, transform=transform_val, download=False, labeled_set=list(range(args.no_known, args.no_class)))
    test_dataset_all_trans = CIFAR100SSL_trans(args.data_root, train_unlabeled_idxs,train=True, transform=transform_val, download=False)
...
class CIFAR10SSL(datasets.CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        return img, target, self.indexs[index],index

class CIFAR10SSL_trans(datasets.CIFAR10):
    def __init__(self, root, indexs, train=True,
                 transform=None, target_transform=None,
...
class CIFAR100SSL_trans(datasets.CIFAR100):
    def __init__(self, root, indexs, train=True,
                 transform=None, target_transform=None,
                 download=False, labeled_set=None):
        super().__init__(root, train=train,
...
class GenericSSL_trans(datasets.ImageFolder):
    def __init__(self, root, indexs,
                 transform=None, target_transform=None,labeled_set=None, no_class=200):
        super().__init__(root, transform=transform, target_transform=target_transform)
        ...
        if labeled_set is not None:
            for i in range(no_class):
                idx = np.where(self.targets == i)[0]
                if i in labeled_set:
                    novel_idx.extend(idx)
            novel_idx = np.array(novel_idx)
            self.data = self.data[novel_idx]
            self.targets = np.array(self.targets)[novel_idx]
